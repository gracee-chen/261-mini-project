...
We benchmark six methods spanning three paradigms: convolutional networks, vision transformers, and classical machine learning. All deep models use ImageNet-pretrained weights (or MIM-pretrained for EVA-02); only the final classification head is replaced with a new linear layer mapping to $C = 102$ classes.
...
\item \textbf{EVA-02-Small}~\citep{fang2024eva} is a vision transformer pretrained via masked image modelling (MIM) on ImageNet-22k and fine-tuned on ImageNet-1k. We load \texttt{eva02\_small\_patch14\_224} from the \texttt{timm} library and replace the classification head ($384 \to C$).
...
\textbf{Differential learning rates.} For each deep model, we partition parameters into backbone (pretrained) and head (randomly initialized) groups with separate learning rates. For \texttt{ResNet-50}, \texttt{EfficientNet-B2}, and \texttt{ConvNeXt-Tiny} we use $\eta_{\text{bb}} = 10^{-4}$ and $\eta_{\text{head}} = 10^{-3}$. For \texttt{ViT-B/16} and \texttt{EVA-02-Small}, which are more sensitive to learning rate, we use $\eta_{\text{bb}} = 2 \times 10^{-5}$ and $\eta_{\text{head}} = 2 \times 10^{-4}$. All models are optimized with AdamW~\citep{loshchilov2017decoupled} ($\lambda = 10^{-4}$).
